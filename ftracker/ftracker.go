...
// duration float64 — длительность тренировки в часах.
// weight float64 — вес человека.
// height float64 — рост человека.
...
func ShowTrainingInfo(action int, trainingType string, duration, weight, height float64, lengthPool, countPool int) string {
	...
	case trainingType == "Бег":
		...
		speed := meanSpeed(action, duration)                       // speed средняя скорость.
		calories := RunningSpentCalories(action, weight, duration) // calories количество потраченных калорий.
		return fmt.Sprintf("Тип тренировки: %s\nДлительность: %.2f ч.\nДистанция: %.2f км.\nСкорость: %.2f км/ч\nСожгли калорий: %.2f\n", trainingType, duration, distance, speed, calories)
	case trainingType == "Ходьба":
		distance := distance(action)                                       // distance пройденная дистанция.
		speed := meanSpeed(action, duration)                               // speed средняя скорость.
		calories := WalkingSpentCalories(action, duration, weight, height) // calories количество потраченных калорий.
		return fmt.Sprintf("Тип тренировки: %s\nДлительность: %.2f ч.\nДистанция: %.2f км.\nСкорость: %.2f км/ч\nСожгли калорий: %.2f\n", trainingType, duration, distance, speed, calories)
	case trainingType == "Плавание":
		distance := distance(action)                                               // distance пройденная дистанция.
		speed := swimmingMeanSpeed(lengthPool, countPool, duration)                // speed средняя скорость.
		calories := SwimmingSpentCalories(lengthPool, countPool, duration, weight) // calories количество потраченных калорий.
		return fmt.Sprintf("Тип тренировки: %s\nДлительность: %.2f ч.\nДистанция: %.2f км.\nСкорость: %.2f км/ч\nСожгли калорий: %.2f\n", trainingType, duration, distance, speed, calories)
	default:
